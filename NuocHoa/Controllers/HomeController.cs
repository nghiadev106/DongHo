...
namespace NuocHoa.Controllers
{
    public class HomeController : Controller
    {
        private readonly NuocHoaContext _context;

        public HomeController( NuocHoaContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> Index()
        {
            ...
            ViewBag.ListProductNam = await _context.Products.Where(a => a.Status == 1 && a.CategoryId==1).OrderBy(x => x.DisplayOrder).Take(5).ToListAsync();
            ViewBag.ListProductNu = await _context.Products.Where(a => a.Status == 1 && a.CategoryId == 2).OrderBy(x => x.DisplayOrder).Take(5).ToListAsync();
